...
async function getInitialData(id: string) {
  ...
  const [purchaseResult, purchaseItemsResult] = await Promise.all([
    executeQuery('SELECT * FROM purchase WHERE purchase_id = ?', [purchaseIdBuffer]),
...
export default async function Page({ params: { id } }: PageProps) {
  return (
    <PurchaseForm initialData={(await getInitialData(id)) as PurchaseFormData} onSubmit={onSubmit} isEditMode={false} />
  );
}
